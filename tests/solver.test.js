...
test('frame: одиночный стержень на растяжение совпадает с δ = FL/(EA)', () => {
  ...
  const loadCase = frameLoadCase(model, { nodalLoads: [[0, 0, 0], [10_000, 0, 0]] })
  const result = analyzeCheckedFrame(model, loadCase, frameParameters)
...
test('frame: консоль с поперечной силой совпадает с δ = PL³/(3EI)', () => {
  ...
  const result = analyzeCheckedFrame(model, frameLoadCase(model, {
    nodalLoads: [[0, 0, 0], [0, forceN, 0]],
  }), frameParameters)
  const inertiaM4 = Math.PI * diameterM ** 4 / 64
  approximately(result.displacements[1][1], forceN * lengthM ** 3 / (3 * 200e9 * inertiaM4), 1e-9)
  approximately(Math.abs(result.rotations[1][2]), forceN * lengthM ** 2 / (2 * 200e9 * inertiaM4), 1e-9)
...
test('frame: консоль инвариантна к повороту в глобальной системе координат', () => {
  ...
  const xResult = analyzeCheckedFrame(xModel, frameLoadCase(xModel, { nodalLoads: [[0, 0, 0], [0, forceN, 0]] }), frameParameters)
  const yModel = singleBeamModel({ lengthM, diameterM, axis: [0, 1, 0] })
  const yResult = analyzeCheckedFrame(yModel, frameLoadCase(yModel, { nodalLoads: [[0, 0, 0], [forceN, 0, 0]] }), frameParameters)
  approximately(Math.hypot(...yResult.displacements[1]), Math.hypot(...xResult.displacements[1]), 1e-9)
})
...
test('frame: жёстко заделанная с двух сторон балка даёт реакции qL/2 и моменты qL²/12', () => {
  ...
  const result = analyzeCheckedFrame(model, frameLoadCase(model, { distributedLoads: [[0, qNPerM, 0]] }), frameParameters)
...
test('frame: чистое осевое растяжение не создаёт паразитного изгиба и кручения', () => {
  ...
  const result = analyzeCheckedFrame(model, frameLoadCase(model, { nodalLoads: [[0, 0, 0], [7000, 0, 0]] }), frameParameters)
  const member = result.memberResults[0]
...
test('frame: глобальное равновесие силы мачты соблюдается', () => {
  ...
  for (let axis = 0; axis < 3; axis += 1) approximately(reaction[axis], -result.loads.totalAppliedLoad[axis], 1e-8, 1e-6)
