...
interface ProcurementWeldConsumable {
  label?: string | null
  process?: string | null
  readonly [key: string]: unknown
}

interface GuyCableGroupInput {
  id?: string | null
  wireId?: string | null
  label?: string | null
  diameterMm?: unknown
  designLengthM?: unknown
  massKgM?: unknown
  source?: string | null
}

export interface ProcurementEstimateInput {
  moduleCount: unknown
  stockBarLengthMm: unknown
  stockBarPieces: unknown
  ribCutLengthMm: unknown
  barDiameterMm: unknown
  moduleDiametersMm?: unknown
  moduleHeightMm?: unknown
  densityKgM3?: unknown
  reservePercent?: unknown
  geometry?: JointGeometry | null
  weldConsumable?: ProcurementWeldConsumable | null
  weldLegMm: unknown
  weldPhysicalLengthPerEndMm: unknown
  boltClass?: unknown
  reinforcementLabel?: unknown
  guyCableGroups?: GuyCableGroupInput[] | null
  guyCableDiameterMm?: unknown
  guyCableLengthM?: unknown
}

interface RebarGroupInput {
  moduleCount: number
  barDiameterMm: number
  moduleDiametersMm?: unknown
  stockBarLengthMm: number
  stockBarPieces: number
  ribCutLengthMm: number
  densityKgM3: number
  reserveFactor: number
}

interface RebarGrouping {
  diameterMm: number
...
const ceilWithReserve = (quantity: number, reserveFactor: number): number => Math.ceil(quantity * reserveFactor - 1e-12)

function hardwareDescription(geometry: JointGeometry, boltClass: unknown) {
  const bolt = geometry.bolt
  const clearance = geometry.bottomClearanceNut
  const coupling = geometry.topCouplingNut
  return {
    bolt: `M${bolt.diameterMm}×${bolt.lengthMm} мм, класс ${String(boltClass)}`,
...
function compactModuleNumbers(moduleNumbers: readonly number[]): string {
  const ranges: string[] = []
...
function buildRebarGroups({
  ...
  const grouped = new Map<number, RebarGrouping>()
...
function buildGuyCableGroups(input: ProcurementEstimateInput, reserveFactor: number) {
  const imported = Array.isArray(input.guyCableGroups) && input.guyCableGroups.length > 0
  const source: GuyCableGroupInput[] = imported
...
export function buildProcurementEstimate(input: ProcurementEstimateInput) {
  ...
  const rebarItems = rebarGroups.map((group) => ({
    ...
    category: 'Арматура',
    specification: `${String(input.reinforcementLabel ?? 'арматура')} Ø${group.diameterMm}; пруток ${stockBarLengthMm / 1000} м → ${stockBarPieces}×${ribCutLengthMm} мм`,
    unit: 'пруток',
...
type ProcurementEstimate = ReturnType<typeof buildProcurementEstimate>
type ProcurementItem = ProcurementEstimate['items'][number]
type RebarGroup = ProcurementEstimate['rebar']['groups'][number]
type GuyGroup = ProcurementEstimate['guyCable']['groups'][number]

const escapeHtml = (value: unknown): string => String(value ?? '')
  .replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;')
  .replaceAll('"', '&quot;').replaceAll("'", '&#39;')
const number = (value: unknown, digits = 2): string => Number.isFinite(Number(value))
...
function estimateRows(estimate: ProcurementEstimate): string {
  return estimate.items.map((item: ProcurementItem) => `
...
function rebarProfileLabel(estimate: ProcurementEstimate): string {
  return estimate.rebar.groups.map((group: RebarGroup) => `Ø${number(group.diameterMm, 0)}: ${group.stockBarCount} прутк.`).join('; ')
}
function guyProfileLabel(estimate: ProcurementEstimate): string {
  return estimate.guyCable.groups.map((group: GuyGroup) => `${group.label ?? `Ø${number(group.diameterMm, 0)} мм`}: ${number(group.procurementLengthM, 2)} м`).join('; ')
}

export function createProcurementEstimateHtml(
  estimate: ProcurementEstimate,
