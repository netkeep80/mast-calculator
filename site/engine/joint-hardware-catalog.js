...
export function metricInternalThreadMinorDiameterMm(diameterMm, pitchMm) {
...
export function getRegularNut(threadDiameterMm) {
  const diameter = Number(threadDiameterMm)
  const nut = REGULAR_NUTS.find((item) => item.threadDiameterMm === diameter)
  if (!nut) throw new Error(`Нет геометрии обычной гайки M${threadDiameterMm}`)
  return nut
}

export function getCouplingNutForBolt(boltDiameterMm) {
  const diameter = Number(boltDiameterMm)
  const nut = COUPLING_NUTS.find((item) => item.threadDiameterMm === diameter)
  if (!nut) throw new Error(`Нет соединительной гайки для болта M${boltDiameterMm}`)
  return nut
}

export function clearanceNutOptionsForBolt(boltDiameterMm, minimumClearanceMm = MIN_CLEARANCE_DIAMETER_MM) {
  const bolt = getBoltSize(boltDiameterMm)
  return REGULAR_NUTS
    .map((nut) => ({
      ...nut,
      basicMinorDiameterMm: metricInternalThreadMinorDiameterMm(nut.threadDiameterMm, nut.pitchMm),
    }))
    .map((nut) => ({
      ...nut,
      diametralClearanceMm: nut.basicMinorDiameterMm - bolt.diameterMm,
    }))
    .filter((nut) => (
      nut.threadDiameterMm > bolt.diameterMm
      && nut.diametralClearanceMm >= minimumClearanceMm
    ))
}

export function minimumClearanceNutForBolt(boltDiameterMm, minimumClearanceMm = MIN_CLEARANCE_DIAMETER_MM) {
  return clearanceNutOptionsForBolt(boltDiameterMm, minimumClearanceMm)[0] ?? null
}

export function minimumStandardBoltLengthMm(requiredLengthMm) {
...
export function buildJointHardwareGeometry(options = {}) {
  ...
  const clearanceNut = options.clearanceNutThreadMm == null
    ...
    : (() => {
        ...
        return {
          ...selected,
          basicMinorDiameterMm,
          diametralClearanceMm: basicMinorDiameterMm - bolt.diameterMm,
        }
      })()
  ...
  const clearancePasses = clearanceNut.threadDiameterMm > bolt.diameterMm
    && clearanceNut.diametralClearanceMm >= minimumClearanceMm
  const engagementPasses = threadEngagementMm + assemblyAllowanceMm <= couplingNut.lengthMm + 1e-9
  const boltLengthPasses = Number.isFinite(boltLengthMm)
    && boltLengthMm + 1e-9 >= minimumRequiredBoltLengthMm
