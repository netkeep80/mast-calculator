...
// В нормах нет универсального закона «старения сварного шва по календарным
// годам». Поэтому это не металлургическая константа, а явный проектный reserve
// model: начальный коэффициент учитывает HAZ/остаточные напряжения и качество
// изготовления, а годовая скорость задаёт дополнительную консервативную
// деградацию. Для реального проекта коэффициенты должны калиброваться по
// материалу, технологии сварки, среде, циклам нагрузки и результатам осмотров.
export function calculateWeldServiceDegradation(options = {}) {
  const serviceYears = Math.max(0, finite(options.serviceYears, DEFAULT_WELD_SERVICE_YEARS))
  const initialRetention = finite(
    options.initialStiffnessRetention,
    DEFAULT_WELD_INITIAL_STIFFNESS_RETENTION,
  )
  const annualLossRate = finite(
