...
interface BandedBucklingOptions {
  tolerance?: number
  maxIterations?: number
  checkEvery?: number
}

interface TridiagonalEigenOptions {
  tolerance?: number
  maxIterations?: number
}

interface TridiagonalEigenpair {
  eigenvalue: number
  vector: number[]
  residual: number
}
...
const scaleMatrix = (matrix: NumericMatrix, scalar: number): number[][] => (
  matrix.map((row) => row.map((value) => value * scalar))
)

export function calculateCriticalBucklingFactor(
  elasticStiffness: NumericMatrix,
  geometricStiffness: NumericMatrix,
): BucklingResult {
  ...
  if (!(eigenpair.eigenvalue > 1e-12)) {
    return {
      ...
      mode: new Array<number>(elasticStiffness.length).fill(0),
      residual: 0,
...
function tridiagonalMultiply(
  diagonal: NumericArray,
  offDiagonal: NumericArray,
  vector: NumericArray,
  shift = 0,
): Float64Array {
...
function largestEigenpairTridiagonal(
  diagonal: NumericArray,
  offDiagonal: NumericArray,
  options: TridiagonalEigenOptions = {},
): TridiagonalEigenpair {
  ...
  for (let index = 0; index < size; index += 1) vector[index] = vector[index]! / norm
...
function kNormalize(vector: NumericArray, elasticStiffness: SymmetricBandMatrix): Float64Array {
...
function reorthogonalizeK(
  vector: Float64Array,
  basis: readonly Float64Array[],
  elasticStiffness: SymmetricBandMatrix,
): void {
...
function buildRitzMode(basis: readonly Float64Array[], coefficients: NumericArray): number[] {
  ...
  for (let basisIndex = 0; basisIndex < basis.length; basisIndex += 1) {
    ...
    const basisVector = basis[basisIndex]!
    for (let index = 0; index < mode.length; index += 1) {
      mode[index] = mode[index]! + factor * basisVector[index]!
    }
  }
  const maximum = Math.max(...mode.map(Math.abs), Number.EPSILON)
...
function bandedBucklingResidual(
  elasticStiffness: SymmetricBandMatrix,
  geometricStiffness: SymmetricBandMatrix,
  mode: NumericArray,
  factor: number,
): number {
...
export function calculateCriticalBucklingFactorBanded(
  elasticStiffness: SymmetricBandMatrix,
  elasticFactorization: SymmetricBandFactorization | SymmetricBandMatrix,
  geometricStiffness: SymmetricBandMatrix,
  options: BandedBucklingOptions = {},
): BucklingResult {
  ...
  let lastEigenpair: TridiagonalEigenpair | null = null
  ...
  if (!(lastEigenpair.eigenvalue > 1e-12)) {
    return {
      factor: Number.POSITIVE_INFINITY,
      mode: new Array<number>(size).fill(0),
      residual: 0,
