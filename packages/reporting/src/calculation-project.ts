...
import { STANDARD_GRAVITY_M_S2 } from '../../engineering/index.js'
import type { VerificationCheck } from '../../engineering/contracts.js'
...
const statusLabel = (status: VerificationCheck['status']): string => status === 'pass' ? 'ПРОЙДЕНО' : status === 'fail' ? 'ОШИБКА' : 'НЕ ПРОВЕРЕНО'
const statusMark = (status: VerificationCheck['status']): string => status === 'pass' ? '✓' : status === 'fail' ? '✗' : '○'

function createLoadAppendix(result: ReportingCalculationResult): string {
...
function createConnectionAppendix(result: ReportingCalculationResult): string {
  const p = result.parameters
...
<div class="formula">
  ...
  <div class="formula-result">${check ? `Nbs=${number(check.shearCapacityN / 1000, 3)} кН; Nbt=${number((check.tensionCapacityN ?? 0) / 1000, 3)} кН; U=${number(check.utilization, 4)}` : '—'}</div>
</div>
<div class="formula">
...
function moduleRows(result: ReportingCalculationResult): string {
...
function createModularHeightAppendix(result: ReportingCalculationResult): string {
...
function verificationCheckHtml(check: VerificationCheck): string {
  return `
<article>
...
function createVerificationAppendix(result: ReportingCalculationResult): string {
