...
test('frame: консоль инвариантна к повороту в глобальной системе координат', () => {
  ...
  const xResult = analyzeFrame(xModel, frameLoadCase(xModel, { nodalLoads: [[0, 0, 0], [0, forceN, 0]] }), frameParameters)
  const yModel = singleBeamModel({ lengthM, diameterM, axis: [0, 1, 0] })
  const yResult = analyzeFrame(yModel, frameLoadCase(yModel, { nodalLoads: [[0, 0, 0], [forceN, 0, 0]] }), frameParameters)
...
test('frame: жёстко заделанная с двух сторон балка даёт реакции qL/2 и моменты qL²/12', () => {
  ...
  const result = analyzeFrame(model, frameLoadCase(model, { distributedLoads: [[0, qNPerM, 0]] }), frameParameters)
...
test('frame: чистое осевое растяжение не создаёт паразитного изгиба и кручения', () => {
  ...
  const result = analyzeFrame(model, frameLoadCase(model, { nodalLoads: [[0, 0, 0], [7000, 0, 0]] }), frameParameters)
  const member = result.memberResults[0]
...
test('ветер вдоль оси отдельного цилиндрического ребра не создаёт поперечной распределённой силы', () => {
  ...
  const result = analyzeFrame(model, frameLoadCase(model), { ...DEFAULT_PARAMETERS, windDirectionDeg: 0, iceThicknessMm: 0 })
