...
## Отдельный поперечный предел / «стрела крана»

Не следует смешивать вертикальную массу вершины с поперечной unit-load задачей. Для оценки конструкции как идеализированной консольной стрелы используется [`LATERAL_CAPACITY_WEATHER_VALIDATION.md`](LATERAL_CAPACITY_WEATHER_VALIDATION.md):

```text
m_crane,ideal = Flateral,limit / g
```

Это эквивалент концевого поперечного груза. В нормированном lateral test собственный вес горизонтально ориентированной стрелы специально исключён, поэтому результат не является паспортной грузоподъёмностью реального крана.
