...
export const circleAreaMm2 = (diameterMm: unknown): number => Math.PI * positive(diameterMm, 'Диаметр') ** 2 / 4
export const hexAreaAcrossFlatsMm2 = (acrossFlatsMm: unknown): number => Math.sqrt(3) * positive(acrossFlatsMm, 'Размер под ключ') ** 2 / 2

export interface NutSectionGeometry {
...
export interface NutSectionOptions {
  requiredRatio?: number
}

export function calculateNutNetSection(
  nut: NutSectionGeometry,
...
export interface JointNutSectionGeometry {
  topCouplingNut: NutSectionGeometry
  bottomClearanceNut: NutSectionGeometry
}

export function checkJointNutSections(
  geometry: JointNutSectionGeometry | null | undefined,
  barDiameterMm: unknown,
  options: NutSectionOptions = {},
) {
  ...
  return {
    method: 'hex-net-section-v1' as const,
    requiredRatio,
    barDiameterMm: Number(barDiameterMm),
    ribAreaMm2: coupling.ribAreaMm2,
    couplingNut: coupling,
    clearanceNut: clearance,
    governingNut: governing === coupling ? 'coupling' as const : 'clearance' as const,
