import type {
  ReportingAnalysis,
  ReportingBuildInfo,
  ReportingCalculationResult,
  ReportingLoadCase,
} from './contracts.js'
import type { GeneratedMastModel } from '../../structural-analysis/index.js'
import type { calculateLateralCapacity, calculateStaticPayloadCapacity, buildVerificationPassport } from '../../engineering/index.js'

type MemberFamily = 'horizontal' | 'diagonal'
type MastMember = GeneratedMastModel['members'][number]
type LateralCapacity = ReturnType<typeof calculateLateralCapacity>
type StaticPayloadCapacity = ReturnType<typeof calculateStaticPayloadCapacity>
type Verification = ReturnType<typeof buildVerificationPassport>
...
const familyName = (family: MemberFamily): string => family === 'horizontal' ? 'Верхний треугольник' : 'Ножка'

function classifyMember(model: GeneratedMastModel, member: MastMember): MemberFamily {
...
function ensureResult(result: ReportingCalculationResult | null | undefined): asserts result is ReportingCalculationResult {
...
export function buildMemberEnvelope(result: ReportingCalculationResult) {
  ensureResult(result)
  ...
  return result.model.members.map((member) => {
    ...
    for (const candidate of candidates.slice(1)) {
      if (candidate.memberResult.utilization > governing.memberResult.utilization) governing = candidate
    }
...
export function buildMaterialSummary(result: ReportingCalculationResult, lengthStepMm = 1) {
  ensureResult(result)
...
const csvNumber = (value: unknown, digits = 3): string => (
...
type ConnectionWeldEnvelopeItem = NonNullable<ReportingCalculationResult['connections']>['weld']['envelope'][number]

function weldLengthMap(result: ReportingCalculationResult): Map<string, ConnectionWeldEnvelopeItem> {
  const map = new Map<string, ConnectionWeldEnvelopeItem>()
...
export function createCalculationCsv(result: ReportingCalculationResult): string {
  const members = buildMemberEnvelope(result)
    .sort((left, right) => ((left.moduleNumber ?? 0) - (right.moduleNumber ?? 0)) || right.utilization - left.utilization)
...
function exportModel(model: GeneratedMastModel) {
  return {
...
function exportModularAnalysis(analysis: ReportingAnalysis) {
...
function exportLoadCase(loadCase: ReportingLoadCase) {
  return {
    windDirectionDeg: loadCase.windDirectionDeg,
    loads: {
...
function exportLateralCapacity(lateralCapacity: LateralCapacity | null | undefined) {
...
function exportStaticPayloadCapacity(staticPayloadCapacity: StaticPayloadCapacity | null | undefined) {
...
function exportVerification(verification: Verification | null | undefined) {
...
function clonePlain<T>(value: T): T {
  ...
  if (ArrayBuffer.isView(value)) {
    return Array.from(value as unknown as ArrayLike<unknown>, (item) => clonePlain(item)) as T
  }
  if (value && typeof value === 'object') {
    return Object.fromEntries(
      Object.entries(value as Record<string, unknown>).map(([key, item]) => [key, clonePlain(item)]),
    ) as T
  }
  return value
}

function exportConnections(connections: ReportingCalculationResult['connections']) {
...
export function createCalculationJson(
  result: ReportingCalculationResult,
  parameters?: ReportingCalculationResult['parameters'],
  generatedAt?: string,
  buildInfo?: ReportingBuildInfo,
): string {
