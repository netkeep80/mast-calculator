export function createSymmetricBandMatrix(size, bandwidth) {
...
export function cloneSymmetricBandMatrix(matrix) {
  return {
    size: matrix.size,
...
function orderedIndices(row, column) {
  return row >= column ? [row, column] : [column, row]
}

export function getBandValue(matrix, row, column) {
...
export function addBandValue(matrix, row, column, value) {
  if (value === 0) return
  const [high, low] = orderedIndices(row, column)
  const offset = high - low
  ...
  matrix.rows[high][offset] += value
}

export function multiplySymmetricBand(matrix, vector) {
  ...
  for (let row = 0; row < matrix.size; row += 1) {
    ...
    result[row] += values[0] * vector[row]
...
export function vectorNorm2(vector) {
  return Math.sqrt(Math.max(0, dotProduct(vector, vector)))
}

export function factorSymmetricBand(matrix) {
...
export function solveSymmetricBandFactor(factorization, rhs) {
  const lower = factorization.lower ?? factorization
...
export function relativeBandResidual(matrix, solution, rhs) {
...
export function denseToSymmetricBand(matrix, tolerance = 0) {
