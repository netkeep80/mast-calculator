...
const textExtensions = /\.(?:js|mjs|cjs|md|html|css|json|txt)$/i
const changed = []
for (const file of tracked) {
  ...
  if (file === 'scripts/foundation52-fix-paths.mjs') continue
  let source = fs.readFileSync(file, 'utf8')
  const original = source

  for (const [oldPath, newPath] of engineMap) source = source.replaceAll(oldPath, newPath)
...
console.log(`Updated moved-path references in ${changed.length} tracked non-workflow files.`)
