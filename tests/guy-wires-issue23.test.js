...
test('issue #23: requested tier height is snapped to an actual module interface node', () => {
...
test('issue #23: 3..6 guys are balanced over the three physical attachment nodes', () => {
  ...
  for (const guyCount of [3, 4, 5, 6]) {
    ...
    for (const cable of system.cables) {
      counts.set(cable.attachmentNodeId, (counts.get(cable.attachmentNodeId) ?? 0) + 1)
    }
    assert.equal(counts.size, 3)
    const distribution = [...counts.values()].sort((left, right) => left - right)
    assert.ok(distribution.at(-1) - distribution[0] <= 1, `${guyCount} guys: ${distribution.join(',')}`)
  }
})

test('issue #23: guyed wind envelope does not reuse bare-mast 120 degree symmetry', () => {
...
test('issue #23: no-guy path reproduces the existing global frame result', () => {
  const p = parameters({
    moduleCount: 6,
    windPressurePa: 300,
  ...
  approximately(
    guyed.envelope.minimumBucklingFactor,
    reference.envelope.minimumBucklingFactor,
    2e-9,
    1e-8,
  )
})
...
test('issue #23: a sufficiently unloaded cable becomes slack instead of taking compression', () => {
  const p = parameters({
    moduleCount: 10,
    extraHorizontalLoadN: 5000,
    windDirectionDeg: 0,
  ...
  assert.ok(guyed.cases[0].nonlinear.converged)
  assert.ok(guyed.cases[0].cables.some((cable) => cable.slack))
