...
test('calculation project identifies normative SP20 load-action provenance', () => {
  ...
  assert.match(html, /γf собственного веса стали[^<]*1,05/)
  assert.match(html, /γf оборудования[^<]*1,05/)
  assert.match(html, /γf гололёда[^<]*1,8/)
  assert.match(html, /γf ветровой нагрузки[^<]*1,4/)
})
...
test('calculation project identifies migrated project/v1 factors without relabelling them normative', () => {
  ...
  assert.match(html, /γf собственного веса стали[^<]*0(?:,0+)?/)
  assert.match(html, /γf оборудования[^<]*1,17/)
  assert.match(html, /γf гололёда[^<]*0(?:,0+)?/)
  assert.match(html, /γf ветровой нагрузки[^<]*1,41/)
})
