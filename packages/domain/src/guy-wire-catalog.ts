export interface GuyWireSpec {
  readonly id: string
  readonly familyId: string
  readonly material: string
  readonly label: string
  readonly diameterMm: number
  readonly metallicAreaMm2: number
  readonly massKgM: number
  readonly minimumBreakingLoadKn: number
  readonly ropeGradeMPa: number
  readonly effectiveYoungModulusGPa: number
  readonly source: string
}

export interface GuyWireCapacityOptions {
  terminationEfficiency?: number
  safetyFactor?: number
}

export interface GuyWireCapacity {
  minimumBreakingLoadN: number
  terminationEfficiency: number
  terminatedBreakingLoadN: number
  safetyFactor: number
  designWorkingLoadN: number
}

const GALVANIZED_DIAMETERS_MM = [4, 5, 6, 8, 10, 12, 14, 16] as const
...
function galvanized6x19SteelCore(diameterMm: number): GuyWireSpec {
...
function stainless7x19(item: { diameterMm: number; massKgM: number; minimumBreakingLoadKn: number }): GuyWireSpec {
...
export const GUY_WIRE_CATALOG: readonly GuyWireSpec[] = Object.freeze([
...
export function getGuyWireSpec(id = DEFAULT_GUY_WIRE_ID): GuyWireSpec {
  const item = GUY_WIRE_CATALOG.find((candidate) => candidate.id === id)
  if (!item) throw new Error(`Неизвестный трос растяжки: ${id}`)
  return item
}

export function calculateGuyWireCapacity(specOrId: GuyWireSpec | string, options: GuyWireCapacityOptions = {}): GuyWireCapacity {
