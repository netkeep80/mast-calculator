...
Прогресс разбит на две части:

```text
78% — перебор стандартных диаметров
...
На GitHub-hosted Ubuntu runner 7 августа 2026 г. измерено:
...
Полный test suite в том же job занял около 1.19 с.
...
4. полная ветровая сетка сворачивается только по доказуемой 120° симметрии;
5. 40-модульная модель остаётся в bounded band (`b <= 35` при текущей топологии/нумерации);
6. main thread не вызывает FEM API;
7. тяжёлая работа выполняется в Worker;
8. UI имеет progress, elapsed, ETA и cancel;
9. static-site smoke test проверяет выдачу worker и banded/buckling modules.
...
Именно поэтому новая реализация проверяется одновременно аналитическими задачами, сравнением с dense reference и прежними regression-тестами физики.