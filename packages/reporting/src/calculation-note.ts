...
const scientific = (value: unknown, digits = 4): string => (
  typeof value === 'number' && Number.isFinite(value)
    ? value.toExponential(digits).replace('.', ',')
    : '∞'
)

const tableRows = (rows: readonly (readonly unknown[])[]): string => rows.map((cells) => (
  `<tr>${cells.map((cell) => `<td>${escapeHtml(cell)}</td>`).join('')}</tr>`
)).join('')

const formula = (symbolic: string, substitution: string, resultText: string): string => `
  <div class="formula">
...
export function createCalculationNoteHtml(
  ...
  const criticalMemberId = strengthCase.analysis.criticalMemberId
  if (criticalMemberId == null) throw new Error('В определяющем случае отсутствует критическое ребро')
  const critical = strengthCase.analysis.memberResults[criticalMemberId]
  if (!critical) throw new Error(`Не найден результат критического ребра ${criticalMemberId}`)
  const criticalMember = result.model.members[critical.memberId]
  if (!criticalMember) throw new Error(`Не найдено критическое ребро ${critical.memberId}`)
