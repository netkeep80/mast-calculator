...
function runOptimization(jobId, parameters) {
  ...
  postProgress(jobId, {
    phase: 'optimize',
    label: `Подбор арматуры и самоконфигурация узла: до ${STANDARD_DIAMETERS_MM.length} стандартных вариантов`,
    fraction: 0,
  })
  ...
  postProgress(jobId, {
    phase: 'done',
    label: `Подбор завершён: Ø${diameter} мм; соединительный узел сконфигурирован автоматически`,
    fraction: 1,
  })
...
self.onmessage = (event) => {
  ...
  try {
    if (action === 'calculate') return runCalculation(jobId, parameters)
    if (action === 'optimize') return runOptimization(jobId, parameters)
