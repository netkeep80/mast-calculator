...
test('расчёт мачты использует v1.0 frame solver, ленточную факторизацию и жёсткий коэффициент расчётной длины', () => {
  const result = calculateMast({
    ...DEFAULT_PARAMETERS,
    moduleCount: 1,
    windEnvelopeEnabled: false,
  ...
  assert.equal(result.method.id, 'linear-frame-v1.0')
