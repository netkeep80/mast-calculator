...
import { solveDenseSystem } from '../../numerics/index.js'
import { analyzeFrame } from '../../structural-analysis/index.js'
...
function booleanCheck({ id, level, title, explanation, passed, evidence, howToCheck }) {
  return {
    id,
    level,
    title,
...
function pendingCheck(id, level, title, explanation, howToCheck) {
...
function geometryAndMaterialChecks(result) {
...
function equilibriumChecks(result) {
  ...
  const reaction = governing.analysis.reactions.reduce(add3, [0, 0, 0])
...
function singleBeamModel({ lengthM, diameterM, axis = [1, 0, 0] }) {
...
function zeroLoadCase(model, nodalLoads) {
  return {
    nodalLoads,
    nodalMoments: model.nodes.map(() => [0, 0, 0]),
    memberDistributedLoads: model.members.map(() => [0, 0, 0]),
    totalAppliedLoad: nodalLoads.reduce(add3, [0, 0, 0]),
    selfWeightN: 0,
    iceWeightN: 0,
    memberWindN: 0,
    equipmentWindN: 0,
...
function analyticalBenchmarkChecks() {
  const parameters = {
  ...
  const axial = analyzeFrame(
    axialModel,
    zeroLoadCase(axialModel, [[0, 0, 0], [axialForceN, 0, 0]]),
    parameters,
  )
  ...
  const bend = analyzeFrame(
    bendModel,
    zeroLoadCase(bendModel, [[0, 0, 0], [0, bendForceN, 0]]),
    parameters,
  )
...
function levelsFromChecks(checks) {
  const definitions = [
  ...
  return definitions.map(([number, title, description]) => {
    ...
    const status = hasFail ? FAIL : hasPending ? PENDING : PASS
    return { number, title, description, status, checkIds: levelChecks.map((check) => check.id) }
  })
}

export function buildVerificationPassport(result) {
