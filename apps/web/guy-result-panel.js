...
function ensurePanel() {
  ...
  panel.innerHTML = `
    <div class="guy-result-heading">
      <div><span class="guy-result-eyebrow">РАСЧЁТ С РАСТЯЖКАМИ</span><h3>Нелинейная cable-envelope</h3></div>
      <strong id="guy-result-status" class="guy-result-status">—</strong>
    </div>
    <div class="guy-result-metrics">
      <article><span>Ребро</span><strong id="guy-member-u">—</strong><small>Umax</small></article>
...
function renderGuyResult(guyResult, bareResult) {
  ...
  status.textContent = guyResult.passes ? 'ПРОХОДИТ' : 'НЕ ПРОХОДИТ'
  ...
  const reduction = Number.isFinite(bareDisplacement) && bareDisplacement > 1e-12
    ...
    : null
  panel.querySelector('#guy-result-summary').textContent = `${guyResult.cableSystem.cables.length} тросов, ${format(guyResult.cableSystem.totalCableLengthM, 1)} м суммарной длины${reduction == null ? '' : `; изменение прогиба относительно голой мачты ${format(reduction, 1)}%`}. Эксплуатационные bare-frame пределы ниже не пересчитываются как guyed capacities.`
...
function renderCombinedPrimaryStatus(snapshot) {
  ...
  const status = answer.querySelector('#scenario-answer-status')
  if (title) title.textContent = snapshot.guyResult.passes
    ? 'Мачта с выбранными растяжками проходит текущие проверки'
    : 'Мачта с выбранными растяжками не проходит текущие проверки'
  if (status) {
    status.textContent = snapshot.guyResult.passes ? 'ПРОХОДИТ' : 'НЕ ПРОХОДИТ'
    status.className = `answer-status ${snapshot.guyResult.passes ? 'pass' : 'fail'}`
  }
}
