...
export function buildEskdConstructionDocumentationModel(result: ReportingCalculationResult) {
  ...
  if (!connections || !geometry) throw new Error('Для экспорта КД отсутствует выбранная геометрия соединительного узла')
  const p = result.parameters
  const completeResult = { ...result, connections }
  const mass = result.assemblyMass ?? calculateAssemblyMass(completeResult)
  ...
  const detailedModel = buildDetailedMastModel(completeResult, { radialSegments: 8, includeJointHardware: true })
  const jointVisual = buildJointVisualGeometry({
    geometry,
    barDiameterMm: Math.max(...memberDiameters(result)),
    weldPhysicalLengthMm: Number(connections.weld?.critical?.check?.requiredPhysicalLengthMm ?? 0),
  })
  ...
  return {
    ...
    boltClass: connections.configurator?.selected?.boltClass ?? p.jointBoltClass ?? '—',
...
type EskdModel = ReturnType<typeof buildEskdConstructionDocumentationModel>
...
function titleBlock(model: EskdModel, name: string, options: TitleBlockOptions = {}): string {
...
function sheet(model: EskdModel, name: string, body: string, options: TitleBlockOptions = {}): string {
...
function drawingSvg(content: string, label: string): string {
...
function mastSpecification(model: EskdModel): string {
...
function moduleSpecification(model: EskdModel): string {
...
function jointDrawing(model: EskdModel): string {
  ...
  const scale = 0.95
  const y0 = 132
  const z = (value: unknown): number => y0 - Number(value) * scale
  const width = (value: unknown): number => Number(value) * scale
...
function ribDrawing(model: EskdModel): string {
...
function standardsNote(): string {
  return ESKD_STANDARDS.map((item) => `${escapeHtml(item.id)} — ${escapeHtml(item.purpose)}`).join('; ')
}

export function createEskdConstructionDocumentation(result: ReportingCalculationResult): string {
...
interface EskdHtmlMetadata {
  source?: unknown
}

export function createEskdConstructionDocumentationHtml(
  result: ReportingCalculationResult,
  metadata: EskdHtmlMetadata = {},
): string {
  const model = buildEskdConstructionDocumentationModel(result)
